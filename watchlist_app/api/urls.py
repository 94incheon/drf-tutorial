...
urlpatterns = [
    ...
    path('<int:pk>/review-create/', views.ReviewCreateAPI.as_view(), name='review-create'),  # 특정영화에 대한 리뷰생성
    path('<int:pk>/reviews/', views.ReviewListAPI.as_view(), name='review-list'),  # 영화에대한 모든리뷰
    path('review/<int:pk>/', views.ReviewDetailAPI.as_view(), name='review-detail'),
